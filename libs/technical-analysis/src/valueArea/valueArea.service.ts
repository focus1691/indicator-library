...
export class ValueAreaService {
  ...
  sumVolumes(klineExchangeKey: IKlineExchangeKey, klines) {
    let V_TOTAL: number = 0
    let highest: number = 0
    let lowest: number = Infinity

    for (let i = 0; i < klines.length; i++) {
      const volume: number = Number(klines[i][klineExchangeKey.volume])
      const high: number = Number(klines[i][klineExchangeKey.high])
      const low: number = Number(klines[i][klineExchangeKey.low])
      V_TOTAL += volume
  ...
  valueAreaHistogram(klineExchangeKey: IKlineExchangeKey, klines, highest: number, lowest: number, nDecimals: number) {
    let row = 0
    ...
    for (let i = 0; i < klines.length; i++) {
      ...
      const high: number = Number(klines[i][klineExchangeKey.high])
      const low: number = Number(klines[i][klineExchangeKey.low])
      const close: number = Number(klines[i][klineExchangeKey.close])
  ...
  getLevelsForPeriod(klineExchangeKey: IKlineExchangeKey, data: any): IValueArea {
    ...
    const { V_TOTAL, high, low }: { V_TOTAL: number; high: number; low: number } = this.sumVolumes(klineExchangeKey, data)
